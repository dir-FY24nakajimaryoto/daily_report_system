...
public class FrontController extends HttpServlet {
	...
	// doPostの中でdoGetを呼び出しているため、POST/GET通信に関わらずdoGetの処理が動作
	 ...
	 * ex) パラメータがaction=Employeeの場合、actions.EmployeeActionオブジェクト
	 * @param request リクエスト
	...
	private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
	    ...
	    try {
	        ...
	        // リクエストからパラメータ"action"の値を取得(ex: "Employee", "Report")
	        String actionString = request.getParameter(ForwardConst.ACT.getValue());
	        
	        // 該当するActionオブジェクトを作成(ex: リクエストからパラメータ action=Employeeの場合、
            ...
            // リクエストパラメータに設定されているactionの値が不正の場合
